...
def create_fixed_length_left_padded_string(context):
    assert(context.source_string == None) is False
    assert(context.padding_character == None) is False
    assert(context.output_length == None or context.output_length == 0) is False
    
    context.left_padded_output = left(context.source_string, context.padding_character, context.output_length)
    print("left_padded_output: " + context.left_padded_output)
    assert(context.left_padded_output == None) is False
